...
class v8supercars_field_post_object extends v8supercars_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	function __construct()
	{
		...
		$this->label = __("Post Object",'v8supercars');
	...
	*/
	...
	function create_field( $field )
	{
		...
		{
			$field['post_type'] = apply_filters('v8supercars/get_post_types', array());
		}
		...
		foreach( $field['post_type'] as $post_type )
		{
			...
			// filters
			$args = apply_filters('v8supercars/fields/post_object/query', $args, $field, $post);
			$args = apply_filters('v8supercars/fields/post_object/query/name=' . $field['_name'], $args, $field, $post );
			$args = apply_filters('v8supercars/fields/post_object/query/key=' . $field['key'], $args, $field, $post );
			...
			if($posts) {
				...
				foreach( $posts as $p ) {
					...
					// filters
					$title = apply_filters('v8supercars/fields/post_object/result', $title, $p, $field, $post);
					$title = apply_filters('v8supercars/fields/post_object/result/name=' . $field['_name'] , $title, $p, $field, $post);
					$title = apply_filters('v8supercars/fields/post_object/result/key=' . $field['key'], $title, $p, $field, $post);
		...
		// create field
		do_action('v8supercars/create_field', $field );
	}
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label for=""><?php _e("Post Type",'v8supercars'); ?></label>
	</td>
	<td>
		<?php 
		...
		$choices = apply_filters('v8supercars/get_post_types', $choices);
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Filter from Taxonomy",'v8supercars'); ?></label>
	</td>
	<td>
		<?php 
		$choices = array(
		...
		$simple_value = false;
		$choices = apply_filters('v8supercars/get_taxonomies_for_select', $choices, $simple_value);
	...
	function format_value_for_api( $value, $post_id, $field )
	{
		...
		if( is_array($value) )
		{
			...
			$posts = get_posts(array(
				'numberposts' => -1,
				'post__in' => $value,
				'post_type'	=>	apply_filters('v8supercars/get_post_types', array()),
				'post_status' => array('publish', 'private', 'draft', 'inherit', 'future'),
			));
...
new v8supercars_field_post_object();
