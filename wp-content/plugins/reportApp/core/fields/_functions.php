...
class v8supercars_field_functions
{
		
	function __construct()
	{
		//value
		add_filter('v8supercars/load_value', array($this, 'load_value'), 5, 3);
		add_action('v8supercars/update_value', array($this, 'update_value'), 5, 3);
		add_action('v8supercars/delete_value', array($this, 'delete_value'), 5, 2);
		add_action('v8supercars/format_value', array($this, 'format_value'), 5, 3);
		add_action('v8supercars/format_value_for_api', array($this, 'format_value_for_api'), 5, 3);
		
		
		// field
		//error_log("add filters\n");
				
		add_filter('v8supercars/load_field', array($this, 'load_field'), 5, 3);
		add_action('v8supercars/update_field', array($this, 'update_field'), 5, 2);
		add_action('v8supercars/delete_field', array($this, 'delete_field'), 5, 2);
		add_action('v8supercars/create_field', array($this, 'create_field'), 5, 1);
		add_action('v8supercars/create_field_options', array($this, 'create_field_options'), 5, 1);
				
		// extra
		add_filter('v8supercars/load_field_defaults', array($this, 'load_field_defaults'), 5, 1);
	}
	...
	*/
	...
	{
		$found = false;
		$cache = wp_cache_get( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], 'v8supercars', false, $found );
		...
		{
			// run filters
			$value = apply_filters('v8supercars/load_value/' . $key . '=' . $field[ $key ], $value, $post_id, $field); // new filter
		}
		
		
		//update cache
		wp_cache_set( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], $value, 'v8supercars' );
	...
	*/
	...
	{
		$value = apply_filters('v8supercars/format_value/type=' . $field['type'], $value, $post_id, $field);
	...
	*/
	...
	{
		$value = apply_filters('v8supercars/format_value_for_api/type=' . $field['type'], $value, $post_id, $field);
	...
	*/
	...
	function update_value( $value, $post_id, $field )
	{
		...
		{
			// run filters
			$value = apply_filters('v8supercars/update_value/' . $key . '=' . $field[ $key ], $value, $post_id, $field); // new filter
		}
	...
	*/
	...
	function delete_value( $post_id, $key )
	{
		...
		wp_cache_delete( 'load_value/post_id=' . $post_id . '/name=' . $key, 'v8supercars' );
	}
	...
	*/
	...
	{
		//error_log("+++++++++++++++++\n");
		//error_log("exe filters\n");
		// load cache
		if( !$field )
		{
			$field = wp_cache_get( 'load_field/key=' . $field_key, 'v8supercars' );
		}
		...
		if( !$field )
		{
			...
			if( !empty($rows) )
			{
				...
				*/
				...
				{
					$wpml_post_id = icl_object_id($row['post_id'], 'v8supercars', true, ICL_LANGUAGE_CODE);
		...
		// apply filters
		$field = apply_filters('v8supercars/load_field_defaults', $field);
		...
		{
			// run filters
			$field = apply_filters('v8supercars/load_field/' . $key . '=' . $field[ $key ], $field); // new filter
		}
		
	
		// set cache
		wp_cache_set( 'load_field/key=' . $field_key, $field, 'v8supercars' );
	...
	*/
	...
	function load_field_defaults( $field )
	{
		...
		$field = apply_filters('v8supercars/load_field_defaults/type=' . $field['type'] , $field);
	...
	*/
	...
	function update_field( $field, $post_id )
	{
		...
		$field = apply_filters('v8supercars/update_field/type=' . $field['post_type'], $field, $post_id ); // new filter
		
		
		// clear cache
		wp_cache_delete( 'load_field/key=' . $field['key'], 'v8supercars' );
	...
	*/
	...
	function create_field( $field )
	{
		...
		//error_log("create field ".$field['type']."\n");
...
(function($) {
	...
	if( typeof v8supercars !== 'undefined' )
	{
		v8supercars.conditional_logic.items.push(<?php echo json_encode($field['conditional_logic']); ?>);
	}
	...
	*/
	...
	function create_field_options($field)
	{
		...
		// render HTML
		do_action('v8supercars/create_field_options/type=' . $field['type'], $field);
	}
	
}

new v8supercars_field_functions();
