...
class v8supercars_field_taxonomy extends v8supercars_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'taxonomy';
		$this->label = __("Taxonomy",'v8supercars');
		$this->category = __("Relational",'v8supercars');
		$this->defaults = array(
			'taxonomy' 			=> 'category',
			'field_type' 		=> 'checkbox',
			'allow_null' 		=> 0,
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {
		...
		// get valid terms
		$value = v8supercars_get_valid_terms($value, $field['taxonomy']);
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		if( $field['load_save_terms'] ) {
			...
			$term_ids = v8supercars_force_type_array( $value );
			...
			if( !did_action('v8supercars/save_post') ) {
			...
			if( empty($this->set_terms) ) {
				...
				// add action
				add_action('v8supercars/save_post', array($this, 'set_terms'), 15, 1);
	...
	*/
	...
	function format_value_for_api( $value, $post_id, $field ) {
		...
		// force value to array
		$value = v8supercars_force_type_array( $value );
	...
	*/
	...
	function create_field( $field )
	{
		...
		$args = array(
			...
			'hide_empty'   => false,
			'style'        => 'none',
			'walker'       => new v8supercars_taxonomy_field_walker( $field ),
		);
		
		$args = apply_filters('v8supercars/fields/taxonomy/wp_list_categories', $args, $field );
		
		?>
<div class="v8supercars-taxonomy-field" data-load_save="<?php echo $field['load_save_terms']; ?>">
	...
	<?php if( $field['field_type'] == 'select' ): ?>
		...
		<select id="<?php echo $field['id']; ?>" name="<?php echo $field['name']; ?>" <?php if( $field['multiple'] ): ?>multiple="multiple" size="5"<?php endif; ?>>
			<?php if( $field['allow_null'] ): ?>
				<option value=""><?php _e("None", 'v8supercars'); ?></option>
			<?php endif; ?>
	
	<?php else: ?>
		<div class="categorychecklist-holder">
		<ul class="v8supercars-checkbox-list">
			<?php if( $field['allow_null'] ): ?>
				<li>
					<label class="selectit">
						<input type="<?php echo $field['field_type']; ?>" name="<?php echo $field['name']; ?>" value="" /> <?php _e("None", 'v8supercars'); ?>
					</label>
				</li>
			<?php endif; ?>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Taxonomy",'v8supercars'); ?></label>
	</td>
	<td>
		<?php
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Field Type",'v8supercars'); ?></label>
	</td>
	<td>
		<?php	
		do_action('v8supercars/create_field', array(
			...
			'optgroup' => true,
			'choices' => array(
				__("Multiple Values",'v8supercars') => array(
					'checkbox' => __('Checkbox', 'v8supercars'),
					'multi_select' => __('Multi Select', 'v8supercars')
				),
				__("Single Value",'v8supercars') => array(
					'radio' => __('Radio Buttons', 'v8supercars'),
					'select' => __('Select', 'v8supercars')
				)
			)
		));
		?>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Load & Save Terms to Post",'v8supercars'); ?></label>
	</td>
	<td>
		<?php	
		do_action('v8supercars/create_field', array(
			...
			'message' => __("Load value based on the post's terms and update the post's terms on save",'v8supercars')
		));
		?>
	</td>
</tr>
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('v8supercars/create_field', array(
			...
			'layout'	=>	'horizontal',
			'choices'	=> array(
				'object'	=>	__("Term Object",'v8supercars'),
				'id'		=>	__("Term ID",'v8supercars')
			)
		));
		?>
	</td>
...
new v8supercars_field_taxonomy();


class v8supercars_taxonomy_field_walker extends Walker
{
	// vars
	var $field = null,
		$tree_type = 'category',
