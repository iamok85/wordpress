...
class v8supercars_field_datetime extends v8supercars_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'datetime';
		$this->label = __("Date Time Picker",'v8supercars');
	...
	*/
	...
	function create_field( $field )
	{
		...
		echo '<div class="v8supercars-date_picker" data-save_format="' . $field['date_format'] . '" data-display_format="' . $field['display_format'] . '" data-first_day="' . $field['first_day'] . '">';		
			echo '<input '.$required.' value="' . $field['value'] . '" name="' . $field['name'] . '" class="input-alt" />';		
		echo '</div>';
		
		?>
		
		<script>					
			jQuery("input[name='<?=$field['name']?>']").datetimepicker({format:'Y-m-d H:m:s'});
		</script>
		<?
	}
...
new v8supercars_field_datetime();
