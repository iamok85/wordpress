...
class v8supercars_field_relationship extends v8supercars_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	function __construct()
	{
		...
		$this->label = __("Relationship",'v8supercars');
	...
	*/
	...
	function create_field( $field )
	{
		...
		$relation_data=$field['relation_data'];
		//debug($relation_data);
		$choices=array();
		
		if(is_array($field['value'])){
			
			foreach($field['value'] as $pid){
			
				$value_str='';
				foreach($relation_data[2] as $per_field){
					
					$content=get_post_meta($pid,$per_field);
					
					if(isset($content[0]))
						$value_str.=$content[0];					
				}
				
				$choices[$pid]=$value_str;
			}
	...
	//	error_log(print_r($relation_data[0],true));
		if(!(isset($relation_data[0])&&$relation_data[0]=='has_many'))
														...
														ajax: {
														    ...
														    data: function (params) {
														      return {
																...
																action : "render_relation_options",
...
new v8supercars_field_relationship();
